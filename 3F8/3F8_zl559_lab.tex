...
\begin{figure}[h]
    \begin{centering}
        \includegraphics[width=0.2\paperwidth]{figures/ll_train_linear.png}\hspace{1cm}\includegraphics[width=0.2\paperwidth]{figures/ll_test_linear.png}
        \par\end{centering}
...
\begin{itemize}
    \item \textbf{Small $l$ (e.g.\ $l = 0.01$):}
          ...
          so the model cannot interpolate between training points.
          The contours confirm this: predictions are highly localised spikes,
          and both log-likelihoods are near $-\ln 2 \approx -0.693$
          (random guessing).
    ...
    \item \textbf{Intermediate $l$ (e.g.\ $l = 0.1$):}
          ...
          structure. The contours show structured probability regions
    ...
    \item \textbf{Large $l$ (e.g.\ $l = 1$):}
          Broad basis functions produce highly correlated features
          (approaching a constant as $l \to \infty$), reducing effective
          dimensionality. The contours are smooth and capture large-scale
          class structure well. Train/test log-likelihoods
...
correlated with large magnitudes, demanding a smaller $\eta$ to avoid
...
matrix biased toward $\hat{y} = 0$ (TP rate only $0.08$) because the
narrow basis functions produce predictions near $0.5$ for test points.

Compared to the linear classifier, all RBF models reduce the false
