...
features $\mathbf{X}$. The log-likelihood is
\[
    \mathcal{L}(\beta)=\sum_{n=1}^{N}\left[y^{(n)}\log\sigma(\beta^{T}\tilde{x}^{(n)})+(1-y^{(n)})\log\left(1-\sigma(\beta^{T}\tilde{x}^{(n)})\right)\right],
\]
where $\sigma(a)=1/(1+e^{-a})$ is the logistic function and $\tilde{x}^{(n)}=(1,x^{(n)})^{T}$
are the bias-augmented inputs. The gradient of the log-likelihood can be
written as
\[
