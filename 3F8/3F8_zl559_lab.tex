...
\usepackage{geometry}
\geometry{verbose,tmargin=1.5cm,bmargin=3cm,lmargin=2.5cm,rmargin=2.5cm}
\usepackage{textcomp}
\usepackage{amstext}
\usepackage{amsmath}
\usepackage{amssymb}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.27\textwidth]{figures/ll_train_rbf_0.01.png}\hspace{0.2cm}
    \includegraphics[width=0.27\textwidth]{figures/ll_test_rbf_0.01.png}\hspace{0.2cm}
    \includegraphics[width=0.27\textwidth]{figures/contours_rbf_0.01.png}\\[-0.1cm]
    \includegraphics[width=0.27\textwidth]{figures/ll_train_rbf_0.1.png}\hspace{0.2cm}
    \includegraphics[width=0.27\textwidth]{figures/ll_test_rbf_0.1.png}\hspace{0.2cm}
    \includegraphics[width=0.27\textwidth]{figures/contours_rbf_0.1.png}\\[-0.1cm]
    \includegraphics[width=0.27\textwidth]{figures/ll_train_rbf_1.0.png}\hspace{0.2cm}
    \includegraphics[width=0.27\textwidth]{figures/ll_test_rbf_1.0.png}\hspace{0.2cm}
    \includegraphics[width=0.27\textwidth]{figures/contours_rbf_1.0.png}
...
\begin{itemize}
    \item \textbf{Small $l$ (e.g.\ $l = 0.01$):}
          ...
          \subitem The decision boundaries are extremely narrow
          and the contour plot shows highly localised, spiky predictive
          probabilities centred on \textit{individual} training points, with no smooth
          interpolation between them. Both training and test log-likelihood values are
    ...
    \item \textbf{Intermediate $l$ (e.g.\ $l = 0.1$):}
          ...
          Experimentally, the basis functions overlap sufficiently
          ...
          central band where Class 1 (red) points dominate. The final train and test
          log-likelihoods are $-0.2433$ and $-0.3537$, respectively -- a
    ...
    \item \textbf{Large $l$ (e.g.\ $l = 1$):}
          ...
          \subitem For $l = 1.0$ (bottom row), the broad basis functions produce smooth
          ...
          two classes. The final train and test log-likelihoods are $-0.3676$
          and $-0.4143$, respectively. The small gap between training and test
          log-likelihoods indicates that the smooth model generalises well
          without overfitting.
\end{itemize}
...
For small $l$, the feature
matrix $\tilde{\mathbf{X}}_{\text{RBF}}$ is approximately the identity, but the gradients have limited magnitude
due to sparse activations, so a moderate learning rate suffices. For intermediate $l$, the features are more correlated and the gradient magnitudes grow (each training point contributes to many
basis functions simultaneously), so a larger learning rate can be used to speed convergence without instability.
For large $l$, the feature matrix is near-singular with very large feature values (all basis functions are close to 1),
producing large gradient magnitudes that demand a smaller learning rate to avoid divergence. Figure~\ref{fig:osc}
demonstrates this: using $\eta = 0.0005$ (10$\times$ larger) for $l = 1.0$ causes severe oscillations -- the
large gradient steps overshoot the optimum on each iteration, causing the log-likelihood to swing wildly rather
than converge monotonically.
...
The RBF expansion with $l = 0.1$ achieves the best test log-likelihood
($-0.3537$), a large improvement over the linear model ($-0.6551$).
Its confusion matrix shows both high true negative rate ($0.93$) and
high true positive rate ($0.79$), meaning it correctly classifies the
majority of both classes. This reflects the ability of intermediate-width
RBFs to capture the non-linear class structure visible in the data.

The $l = 1.0$ model has a slightly worse test log-likelihood ($-0.4143$)
but the best true positive rate ($0.875$), at the cost of a higher
false positive rate ($0.21$). The smooth, broad basis functions capture
the large-scale separation between classes but lack the resolution
to model finer boundaries, leading to more class-0 points being
misclassified as class-1.

The $l = 0.01$ model performs poorly: its test log-likelihood ($-0.6927$)
is close to that of random guessing ($-\ln 2 \approx -0.693$). Its
confusion matrix is heavily biased toward predicting $\hat{y} = 0$
(true negative rate $0.96$ but true positive rate only $0.08$),
because the extremely narrow basis functions produce predictions near
$0.5$ for most test points, and the threshold $\tau = 0.5$ then
assigns nearly all predictions to class 0.

Compared to the linear classifier, all three RBF models reduce the
false positive rate (the linear model has $0.38$). However, only
$l = 0.1$ and $l = 1.0$ simultaneously maintain a high true positive
rate, confirming that a non-linear feature expansion with an
appropriately chosen width substantially improves classification on
this dataset.
