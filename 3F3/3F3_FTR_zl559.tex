...
\subsection{Explanation of Multinomial Distribution Form}
...
\textbf{Why the $p_j^{n_j}$ terms?} Each sample is an independent trial. For a specific sample to land in bin $j$, the probability is $p_j$. Since all $N$ samples are independent, the probability of a specific sequence of outcomes is the product of individual probabilities. If exactly $n_j$ samples land in bin $j$, these contribute a factor of $p_j^{n_j}$.

\textbf{Why the factorial terms?} The factorial coefficient counts the number of distinct arrangements that yield the same bin counts. Consider $N$ labelled samples: the number of ways to choose which $n_1$ samples go to bin 1, then which $n_2$ of the remaining go to bin 2, and so forth, is:
\begin{equation}
