...
\begin{equation}
    \E[\Pi_t^{(k,j)}] = 12\Bigl(p\mu_t^{(j)} - c_p^{(k)}\, Q_t^{*(k,j)} - c_s^{(k)}\,\mathrm{ELS}(Q_t^{*(k,j)}) - c_h\,\mathrm{ELOI}(Q_t^{*(k,j)})\Bigr)
    \label{eq:annual_profit}
\end{equation}
where $\mu_t^{(j)}$, $\sigma_t^{(j)}$ come from Table~\ref{tab:demand}, and $c_s^{(k)}$, $c_p^{(k)}$, $\bar{Q}^{(k)}$ are from Table~\ref{tab:strategy_params}.
