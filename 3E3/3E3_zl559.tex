...
\begin{equation}
    \E_D[\Pi(Q_t^*)] = p\mu_t - c_p Q_t^* - c_s\,\text{ELS}(Q_t^*) - c_h\,\text{ELOI}(Q_t^*)
    \label{eq:exp_profit}
\end{equation}
...
Table~\ref{tab:nv_annual} summarises expected annual profit $\E[\Pi_t^{(k,j)}]$ under each strategy $k \in \{S1, S2, S3\}$, scenario $j \in \{\textit{High-Growth+No Competition, High-Growth+Competition, Low-Growth}\}$, and year $t \in \{0,1,2,3,4\}$. We recalculate $\E[\Pi_t^{(k,j)}]$ based on the modelling changes made to our parameters: effective capacity $\bar{Q_k}$ under strategy $k$, $sigma_t$, $c_s$ and $c_p$, and revenue cuts etc.
