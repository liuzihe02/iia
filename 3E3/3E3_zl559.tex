...
The three strategies are evaluated over a 4-year horizon using a multi-stage decision tree (Figure~\ref{fig:tree}). Strategy~3 has a real-options structure: at the end of Y1, GreenCo observes realised demand growth and selects a branch. If high growth is observed, automatically expand capacity (\pounds42M, matching S1 from Y2); if low growth, invest in flexibility (\pounds15M, matching S2 from Y2). The tree in Figure~\ref{fig:tree} shows this resolved optimal policy directly.
...
\begin{equation}
    ...
    \label{eq:npv}
\end{equation}
where $I_0^{(k)}$ is the upfront capital expenditure (Table~\ref{tab:strategy_params}), $I_1^{(k,j)}$ is the conditional Year-1 investment discounted at $t=1$ (applicable S3 only), and annual profits $\E[\Pi_t^{(k,j)}]$ are from Table~\ref{tab:nv_annual}.
...
\textbf{Strategy 2 dominates by a large margin in every scenario.} Its E[NPV] of \pounds46.5M exceeds S1 by \pounds39.9M and S3 by \pounds25.0M. The mechanism is threefold: (i)~much lower capital expenditure (capex) (\pounds20M vs \pounds54M/\pounds50M equivalent) leaves less invested capital to recover; (ii)~reduced $c_s$ from \pounds3200 to \pounds2900/t lowers the underage penalty, shifting the critical ratio from 0.200 to 0.125 and reducing expected spot procurement costs; (iii)~reduced demand volatility ($\sigma \to 0.6\sigma$) lowers both expected leftover and expected shortfall, simultaneously reducing storage and procurement waste.
...
\begin{table}[H]
    \centering
    \caption{Monte Carlo risk measures ($N = 50000$, all values in £M except $\Prob(\mathrm{NPV}<0)$). Conditional Value-at-Risk ($\mathrm{CVaR}_{5\%}$): expected NPV in the worst 5\% of outcomes.}
    \label{tab:risk}
    \small
    \begin{tabular}{@{}lccc@{}}
        \toprule
...
Each strategy's NPV distribution (Figure~\ref{fig:npv_dist}) is a mixture of three Gaussians, one per terminal scenario weighted by scenario probability (0.27, 0.18, 0.55). Within each scenario, monthly demand randomness produces a Gaussian-shaped NPV cluster; the separation between clusters reflects the scenario-driven differences in annual profits.

Strategy 2 is superior on \textit{every} risk dimension simultaneously---it has the highest E[NPV], the \textit{lowest} standard deviation, the highest CVaR, and zero probability of a net loss. Its three scenario clusters are tightly grouped in the \pounds42--53M range (Figure~\ref{fig:npv_dist}), with little overlap---even the worst-case cluster (low growth) sits above \pounds40M.

Strategy 1 has a 13.8\% probability of a net loss. Its low-growth cluster (weight 0.55) is centred near \pounds1M with left-tail mass crossing zero, while the two high-growth clusters sit around \pounds10--16M. The wide spread (std.\ \pounds6.71M) reflects large inter-scenario separation compounded by within-scenario demand variability.

Strategy 3 has zero probability of loss but a distinctly bimodal distribution (std.\ \pounds10.51M): the dominant low-growth cluster (weight 0.55) sits near \pounds31M, while the two high-growth clusters are compressed around \pounds7--12M. The real option is more valuable under \textit{low} growth---flexibility investment is cheap and the expensive capacity expansion is avoided.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{fig_npv_dist.pdf}
    \caption{Simulated NPV density for each strategy (dashed lines = E[NPV]). Each distribution is a mixture of three Gaussian clusters corresponding to the three terminal scenarios. S2's clusters are tightly grouped above \pounds40M; S1's low-growth cluster crosses zero; S3 is bimodal with the dominant low-growth peak near \pounds31M.}
    \label{fig:npv_dist}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Adopt Strategy 2 (long-term investment decision).} S2 achieves the highest E[NPV] of \pounds46.5M (vs \pounds6.6M for S1, \pounds21.6M for S3), zero probability of net loss, lowest NPV volatility, and highest CVaR. It dominates in every scenario (Table~\ref{tab:npv}). The mechanism: lower capex (\pounds20M), reduced spot cost ($c_s = \text{\pounds}2900$/t shifting the critical ratio from 0.200 to 0.125), and reduced demand volatility ($\sigma \to 0.6\sigma$) jointly cut procurement and storage waste. Under high growth, S2 relies on spot procurement from Y3, but pre-negotiated contracts at \pounds2900/t provide downside protection; even in this worst case, S2's NPV (\pounds48--53M) far exceeds alternatives.

    \item \textbf{Implement newsvendor-optimal production (performance stability).} Set $Q_t^* = \min(\mu_t + z^*\sigma_t,\,\bar{Q})$, reviewed annually. Planning at mean demand ignores the asymmetric cost structure ($c_o \gg c_u$) and the flaw of averages, leading to systematic overproduction and unnecessary cost exposure.

    \item \textbf{Add two refuelling bays immediately (service-level compliance).} The current M/M/3 configuration violates the 5-minute SLA at baseline ($W_q = 5.33$ min) and deteriorates rapidly under demand growth. Two additional bays ($s = 5$) are required for compliance across the full horizon ($W_q \leq 1.27$ min). A single bay ($s = 4$) is insufficient under high growth by Y4. This is independent of the investment strategy chosen.

    \item \textbf{Monitor demand and maintain adaptive capacity (resilience).} If high growth materialises earlier than expected, activating an option to expand capacity (analogous to S3) may become worthwhile from Y2. GreenCo should also secure spot contract volumes sufficient to cover projected shortfalls under high growth and establish fallback suppliers to mitigate counterparty risk.
\end{enumerate}

\subsection{Assumptions and Limitations}

\begin{itemize}
    \item \textbf{S3 specification matching:} The coursework does not specify availability or demand parameters for S3's conditional investments. We assume S3's expansion branch matches S1 specifications (95\% availability, 65 t/day) and S3's flexibility branch matches S2 specifications ($\sigma \to 0.6\sigma$, $c_s = \text{\pounds}2900$/t).
    \item \textbf{Arrival rate scaling:} HGV arrival rate $\lambda_t$ is assumed to scale with mean hydrogen demand growth. This links the queueing and newsvendor models but is not specified in the problem.
    \item \textbf{Demand independence:} Monthly demands are assumed i.i.d.\ within each year. Serial correlation would alter the newsvendor solution and widen the simulated NPV distribution.
    \item \textbf{Risk neutrality:} The analysis maximises E[NPV]. A risk-averse decision-maker would weight downside outcomes more heavily, strengthening the case for S2 further.
    \item \textbf{Queueing model:} Steady-state M/M/$s$ assumes stationary Poisson arrivals. Time-of-day arrival patterns would give a more accurate picture of peak congestion.
\end{itemize}
