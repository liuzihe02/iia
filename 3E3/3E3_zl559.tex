...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        $\mu_0$            & Initial mean monthly demand                       & 1{,}000 t                    \\
...
Monthly hydrogen demand in year $t$: $D_t \sim \mathcal{N}(\mu_t, \sigma_t^2)$, with constant $\mathrm{CV} = \sigma_t/\mu_t = 0.15$, and annual growth rate of mean demand $g$ gives:
\begin{equation}
...
Each month GreenCo chooses production quantity $Q \in [0,\bar{Q}]$ before demand $D \sim \mathcal{N}(\mu,\sigma^2)$ is realised. The effective monthly capacity $\bar{Q}$ is the product of nominal electrolyser throughput, number of bays, operational availability, and operating days:
\begin{equation}
    \bar{Q} = K_{\mathrm{nom}} \cdot s_0 \cdot a \cdot n_d = 1{,}020 \text{ t/mo}
    \label{eq:capacity}
\end{equation}
...
\begin{align}
    ...
    c_o & = c_p + c_h = \text{\pounds}2{,}800/\text{t} &  & \text{(production + storage on unsold units)}
\end{align}

The ratio $c_o/c_u = 4$ is strongly asymmetric: overproducing is four times more expensive per tonne than underproducing. Therefore $F(Q^*) = c_u/(c_u+c_o) = 700/3{,}500 = 0.20$. For normal demand $Q^* = \mu + z^*\sigma$ where $z^* = \Phi^{-1}(0.20) = -0.8416$. The firm optimally produces \textit{well below} mean demand, covering the shortfall from the relatively cheap spot market. Base-case (at $t=0$):
\begin{equation}
    Q_0^* = \mu_0 + z^*\sigma_0 = 1{,}000 + (-0.8416)(150) \approx 874 \text{ t/month} \quad < \bar{Q} = 1{,}020
\end{equation}
...
For normal demand, the standard normal loss function $L(z) = \phi(z) - z[1-\Phi(z)]$ gives closed-form expressions for ELS and ELOI. With $z^* = -0.8416$ and $L(-0.8416) = 0.953$:
\begin{align}
...
Using Eqn ~\eqref{eq:exp_profit}, our expected profit at $t=0$ is:
\begin{equation}
