...
\begin{figure}[H]
    ...
    \resizebox{0.85\linewidth}{!}{%
        \begin{tikzpicture}[
            ...
            % ── S3: H/L split ─────────────────────────────────────
            \node[decision] (s3D) at ( 5.2,-4.8) {Y1 Decision};
            \node[terminal] (s3L) at ( 9.2,-4.8) {};
            \node[right=2pt of s3L] {\pounds30.8M};
            \draw[edge] (s3) -- node[left,  align=center, xshift=-8pt] {High Growth\\($p_H{=}0.45$)} (s3D);
            \draw[edge] (s3) -- node[right, align=center, xshift= 8pt] {Low Growth\\($1{-}p_H{=}0.55$)} (s3L);
            % S3 Y1 Decision -> Expand / Flex
            \node[chance]   (s3E) at (3.5,-7.2) {};
            \node[terminal] (s3F) at (7.5,-7.2) {};
            \node[right=2pt of s3F] {\pounds30.8M};
            \draw[edge] (s3D) -- node[left,  xshift=-6pt] {Expand Capcity} (s3E);
            \draw[edge] (s3D) -- node[right, xshift= 6pt] {Invest in Flexibility}   (s3F);
            % S3 Expand -> nc/c
            \node[terminal] (s3nc) at (1.8,-9.4) {};
            \node[terminal] (s3c)  at (5.2,-9.4) {};
            \node[right=2pt of s3nc] {\pounds12.4M};
            \node[right=2pt of s3c]  {\pounds7.0M};
            \draw[edge] (s3E) -- node[left,  align=center, xshift=-8pt] {No Comp.\\($1{-}p_C{=}0.60$)} (s3nc);
            \draw[edge] (s3E) -- node[right, align=center, xshift= 8pt] {Comp.\\($p_C{=}0.40$)} (s3c);

            % ── Legend ────────────────────────────────────────────
            \node[font=\small] at (0,-10.2) {%
        ...
        \end{tikzpicture}}
    \caption{Decision tree for the three strategies. Terminal node values are NPVs. S3 has a real-options Y1 Decision node (conditional on observed growth) on the high-growth branch: Expand matches S1 specifications, Flex matches S2 specifications. EMV values (blue) computed by backward induction.}
    \label{fig:tree}
\end{figure}
