...
def encode(x, p):
    """
    Encode a sequence using arithmetic coding with finite precision.

    This implementation uses integer arithmetic with rescaling to avoid
    precision loss. The algorithm maintains an interval [lo, hi) and outputs
    bits when the interval boundaries share common leading bits or straddle
    the half-way point.

    Parameters:
    -----------
    x : list or iterable
        Input sequence of symbols to encode. Each symbol must be in the
        probability distribution p.
    p : dict
        Probability distribution mapping symbols to their probabilities.
        Must sum to 1.0. Zero probabilities are filtered out.

    Returns:
    --------
    y : list of int
        Binary encoded sequence as a list of 0s and 1s.
    """
    ...
    for k in range(len(x)):  # for every symbol
        ...
        while True:
            ...
            hi = (
                2 * hi + 1
            )  # I DON'T KNOW WHY +1 IS NECESSARY BUT IT IS. THIS IS MAGIC.
