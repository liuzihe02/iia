...
\begin{frame}{Underwater Communication Challenges}
  ...
  \note{
    Radio frequency signals attenuate extremely rapidly in seawater due to conductivity. Skin depth at WiFi frequencies is less than 10 centimeters. Effective RF range underwater is only a few meters maximum, making it useless for cable inspection missions.

    Optical communication uses laser or LED light transmission. Water absorbs and scatters light, with attenuation increasing significantly in turbid or murky conditions. Optical systems require direct line-of-sight and operate at ranges under 100 meters in clear water, much shorter in realistic ocean environments with sediment and particulates.

    Acoustic communication is the only viable long-range option. Sound propagates well in water with ranges exceeding several kilometers. However, acoustic modems have severe limitations: data rates around 1 to 10 kilobits per second, latency of seconds due to slow sound speed, and interference from reflections off the surface and seafloor. Cost is also prohibitive - acoustic modems cost 12,000 dollars versus 310 dollars for WiFi plus Iridium satellite.

    The design solution stores most sensor data onboard during the mission. High-resolution camera video and sonar scans are recorded to the Raspberry Pi storage. At the surface, WiFi provides high-bandwidth data transfer at 50 megabits per second. Iridium satellite enables position reporting and mission status updates when beyond WiFi range. This approach avoids expensive acoustic hardware while maintaining operational capability.
  }
\end{frame}
...
\begin{frame}{Navigation System}
  ...
  \note{
    The Navigator is the core stabilization board. Its dual IMUs are fused by the software to reduce sensor noise and drift.

    The Raspberry Pi 4 is the "brain" or main computer. It runs the high-level autopilot software and processes all the mission and sensor data.

    The Bar30 sensor just measures water pressure, which gives us a very accurate vertical position (depth).

    For horizontal position, GPS only works on the surface. The Surface GPS is used to get a precise starting point before the drone dives.
    Once underwater, the drone uses IMU dead reckoning —it estimates its position by tracking its own movement from that GPS starting point. This estimation is imperfect and accumulates errors, which causes the 5-15m drift over the mission.
  }
\end{frame}
...
\begin{frame}{Underwater Acoustic Link Budget}
  ...
  \note{
    This link budget calculation demonstrates that underwater acoustic communication is technically feasible
...
\begin{frame}{Hydrodynamic Drag}
  ...
  \begin{itemize}
    ...
    \item Drag coefficient: $C_D = 0.28$-$0.32$ \href{https://www.mdpi.com/2311-5521/6/7/252}{\textsuperscript{[MDPI CFD]}}
  \end{itemize}
...
\begin{frame}{Design Conclusions}
  ...
  \note{
    ...
    Constraints: The low-drag hull requirement is non-negotiable. If drag coefficient increases from 0.30 to 0.40, drag force at 4 meters per second increases from 162 to 216 newtons, requiring 270 newtons total thrust. Current four T200 configuration provides only 200 newtons, insufficient. IMU dead reckoning accumulates significant drift over 2 hours. Without (DVL) Doppler Velocity Logger providing velocity measurements, horizontal position uncertainty grows, requiring periodic surface GPS fixes for position correction.
  }
\end{frame}
