...
\begin{frame}{Existing Commercial Solutions}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.24\textwidth}
      ...
      \tiny Boxfish: 8 thrusters, 6-DOF
    \end{column}
    \begin{column}{0.24\textwidth}
      \centering
      \textbf{BlueROV2}
      \tiny Reference platform (tethered ROV). Proves component viability
    \end{column}
  \end{columns}

  \vspace{0.3em}
  \centering
  \colorbox{yellow!30}{\small\textbf{Key finding: No commercial AUV <25 kg achieves 4 m/s sustained speed}}
...
\begin{frame}{Complete System Power Budget}
  \begin{table}
    ...
    \begin{tabular}{lccc}
      ...
      \textbf{[PAYLOAD] Camera}   & 1.1                 & 1.1               & Low-light USB            \\
      \textbf{[PAYLOAD] Sonar}    & 2                   & 5                 & Ping360 scanning         \\
      \textbf{[PAYLOAD] Lighting} & 10                  & 20                & 2× Lumen adjustable      \\
      \textbf{PAYLOAD SUBTOTAL}   & \textbf{13.1}       & \textbf{26.1}     & \textbf{Within 30W spec} \\
      \midrule
      Navigation sensors          & 5                   & 5                 & IMU, depth, GPS          \\
      Control (RPi4+Nav)          & 10                  & 10                & ArduSub firmware         \\
      Comms (WiFi/Iridium)        & 2                   & 2                 & Surface only             \\
      \midrule
      \textbf{TOTAL}              & \textbf{75 W}       & \textbf{1,415 W}  &                          \\
      \bottomrule
    \end{tabular}
  \end{table}
  ...
  \textbf{Mixed Mission Profile} (60\% cruise + 30\% medium @216W + 10\% peak):
  $$P_{avg} = 0.6 \times 75 + 0.3 \times 216 + 0.1 \times 1{,}415 = 251 \text{ W}$$
...
\begin{frame}{Communications Systems: Actual Products}
  ...
  \begin{itemize}
    ...
    \item \textbf{Underwater (optional):} Acoustic modem for real-time telemetry
  \end{itemize}
