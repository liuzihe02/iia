...
As a result, our design minimizes underwater communication. Instead, we store all sensor data onboard during the mission and transfer it to the surface once the vehicle resurfaces. This approach maximizes data throughput while avoiding the complexities of underwater comms.
...
\begin{frame}{Communications Systems}
  ...
  \note{
    \textbf{WiFi 802.11n - WHY THIS CHOICE:}
    
    The Raspberry Pi 4 includes built-in WiFi at zero additional hardware cost - we only need a 50-dollar marine-grade antenna with waterproof connector. When the vehicle surfaces, WiFi provides 150 megabits per second bandwidth enabling complete mission data download in 2-5 minutes. Our 2-hour mission generates approximately 7-8 gigabytes of 1080p video plus 20-25 gigabytes of Ping360 sonar data if recording continuously. At 150 megabits per second (approximately 18 megabytes per second real throughput), we can transfer 2-4 gigabytes in 2-5 minutes. The operational strategy is to surface every 30 minutes, dump accumulated data via WiFi, then continue the mission. This eliminates the need for expensive onboard storage (a 512 gigabyte industrial SSD costs 200-400 dollars) and ensures data is safely transferred even if the vehicle is lost. WiFi range of 50-100 meters at the surface is adequate when operating near the mothership. For operations beyond WiFi range, the vehicle stores data locally and dumps it when returning to the ship.
    
    \textbf{RockBLOCK 9603N Iridium - WHY THIS CHOICE:}
    
    The RockBLOCK provides global satellite coverage pole-to-pole, critical for open ocean operations where the vehicle may travel kilometers from the mothership following the cable route. Every 10 minutes, the vehicle surfaces and transmits a 340-byte message containing GPS coordinates (latitude, longitude, ~20 bytes), depth, heading, battery voltage, and status flags. This costs approximately 11-14 cents per message, so 12 messages over a 2-hour mission costs 1.32-1.68 dollars in data charges. The monthly line rental is 15 dollars whether we use it or not. This small operational cost ensures vehicle recoverability - if the vehicle loses communication or experiences a fault, the last-known position enables recovery. The RockBLOCK draws only 0.8 watts average during message transmission (20 second session) and 40 milliamps idle (0.2 watts), making power impact negligible. At 260 dollars hardware cost, it's 30-50 times cheaper than marine satellite terminals costing 8,000-15,000 dollars.
    
    \textbf{WHY NOT ACOUSTIC MODEM (12,000 dollars):}
    
    Acoustic modems like the EvoLogics S2C M 18/34 cost 12,000-15,000 dollars and provide 13.9 kilobits per second data rate. Transferring our 7-8 gigabytes of video data would require 72-80 hours at full acoustic bandwidth - completely impractical. Even if we only sent status telemetry (100 bytes every 10 seconds = 1 kilobyte per minute = 120 kilobytes over 2 hours), this represents only 0.0017 percent of our total data volume. The acoustic modem costs 40 times our entire communication budget (WiFi plus Iridium = 310 dollars) yet provides no practical benefit for our autonomous mission profile. Acoustic modems are justified for tethered ROVs requiring real-time video and control, or for multi-vehicle coordination. Our vehicle follows pre-programmed waypoints, stores data onboard, and surfaces for communication - eliminating acoustic modem necessity.
    
    \textbf{WHY NOT OPTICAL MODEM (8,000-12,000 dollars):}
    
    Underwater optical communication achieves megabit per second data rates but requires line-of-sight and operates only in clear water under 50-100 meters range. Subsea cable inspection occurs in turbid coastal waters where visibility may be 1-3 meters. Optical modems cost 8,000-12,000 dollars and would be useless in our operating environment. Even in clear water, the vehicle must maintain precise alignment with the mothership's optical transceiver - incompatible with autonomous cable-following missions.
    
    \textbf{DATA VOLUME ANALYSIS - JUSTIFYING SURFACE COMMUNICATION:}
    
    Camera: 1080p at 30 fps, H.264 compression = 8 megabits per second = 1 megabyte per second = 3.6 gigabytes per hour = 7.2 gigabytes for 2-hour mission.
    
    Ping360 sonar: 400 points per scan, 10-20 hertz scan rate, 16-bit data = approximately 12-24 kilobits per second = 1.5-3 megabytes per minute = 90-180 megabytes per hour. If we record every scan, this is 180-360 megabytes for 2 hours - much less than video.
    
    Total: approximately 7.5-8 gigabytes per mission. At 150 megabits per second WiFi throughput (18-19 megabytes per second), transfer time is 7,500 megabytes divided by 18 = 417 seconds = 7 minutes. If we surface every 30 minutes (four times during 2-hour mission), each surface interval transfers approximately 2 gigabytes in 2 minutes, well within our operational margin.
    
    \textbf{ALTERNATIVE APPROACH REJECTED - ONBOARD STORAGE ONLY:}
    
    We could store all data on a 512 gigabyte SSD and download after mission completion. This saves 2-minute surface intervals but introduces risk - if the vehicle is lost, all mission data is lost. The surface-and-dump strategy ensures data is safely transferred even if subsequent vehicle loss occurs. The 2-minute surface intervals every 30 minutes also allow GPS position resets that reduce IMU drift, making them operationally beneficial rather than purely communication-driven.
...
\begin{frame}{Navigation System Component Options}
  ...
  \begin{table}
    ...
    \begin{tabular}{lllll}
      ...
      \midrule
      \textbf{Flight Controller} & Navigator (dual IMU)        & 2.8°/s/$\sqrt{\mathrm{Hz}}$ gyro noise        & \$125              \\
                                 & Pixhawk 6C                  & Similar performance          & \$300                        \\
      \midrule
      \textbf{Depth Sensor}      & Bar30 (MS5837)              & 2mm resolution, ±2m accuracy & \$68              \\
                                 & Keller PA-7LD               & ±0.3m accuracy, stable       & \$200-500                    \\
      \midrule
      \textbf{Surface GPS}       & u-blox NEO-M8N              & 2.5m accuracy                & \$35              \\
                                 & RTK GPS                     & <0.1m accuracy               & \$300-600            \\
      \midrule
      \textbf{DVL}    & Nortek DVL1000              & 0.2 cm/s, <1m over 2 hrs     & \$20,000             \\
  ...
  \textbf{Baseline Cost: \$20208} (Navigator + Bar30 + GPS + DVL + RPi4)

  \note{
    \textbf{Navigator Flight Controller - Component Justification:}

    The Navigator provides dual IMUs: ICM-42688-P with 2.8 millidegrees per second per root hertz gyro noise (industry-leading for MEMS), and ICM-20602 backup with 0.015 degrees per second per root hertz. Dual magnetometers (BMM150) provide heading reference with redundancy. The Navigator integrates directly with Raspberry Pi 4 via 40-pin GPIO header, eliminating external cabling and reducing failure points inside the pressure housing. At 125 dollars, it costs 60 percent less than Pixhawk 6C (300 dollars) while providing equivalent or better sensor specifications. The Navigator is the native platform for ArduSub autopilot with proven reliability in thousands of BlueROV2 deployments worldwide.

    \textbf{Bar30 Depth Sensor - Why Adequate for Our Mission:}

    The Bar30 uses the MS5837-30BA pressure sensor providing 2 millimeter depth resolution and 2 meter absolute accuracy. For cable inspection missions where we maintain 2 to 5 meter altitude above the seabed, 2 meter accuracy is entirely adequate - we're not doing bathymetric surveys requiring centimeter precision. The limitation is that the sensor gel can absorb water over multi-day deployments, causing drift. Our 2-hour missions with daily topside access allow drying the sensor, eliminating this concern. Industrial alternatives like Keller PA-7LD provide 0.3 meter accuracy and superior long-term stability but cost 3 to 7 times more (200 to 500 dollars). The Bar30's I2C interface and native ArduSub support make integration trivial. At 68 dollars, it's exceptional value for short-duration applications.

    \textbf{GPS Selection - Adequate Accuracy for Drift Reset:}

    The u-blox NEO-M8N provides 2.5 meter horizontal accuracy in open ocean conditions. We use GPS only at the surface to reset accumulated IMU drift every 30 minutes. The 2.5 meter GPS error is negligible compared to 5 to 15 meter IMU drift over 30-minute intervals. RTK GPS achieves centimeter-level accuracy but costs 300 to 600 dollars - unnecessary when GPS is used for coarse position reset rather than continuous high-precision tracking. The NEO-M8N is a standard industrial module with proven reliability at 35 dollars.

    \textbf{DVL - When is the Upgrade Justified:}

    Doppler Velocity Logs measure water velocity relative to the seabed using acoustic Doppler effect. The Nortek DVL1000 provides 0.2 centimeters per second velocity accuracy, enabling sub-meter position accuracy over 2-hour missions. It costs 15,000 to 25,000 dollars and weighs 1.2 kilograms. When is this justified? Three scenarios: First, survey missions requiring precise bathymetry or cable route mapping where absolute position matters. Second, zero-visibility operations where visual cable tracking is impossible (heavy sediment, nighttime). Third, multi-vehicle coordination requiring precise relative positioning between AUVs. For basic cable inspection following the cable visually, the DVL cost is unjustified - it represents twice our entire baseline build cost.

    Teledyne RDI Pathfinder is the alternative DVL with similar performance and extensive OEM support. Cost is 18,000 to 28,000 dollars. Choice between Nortek and Teledyne depends on availability and support requirements.

    \textbf{Baseline Navigation Cost Analysis:}

    Navigator 125 plus Bar30 68 plus GPS 35 plus Raspberry Pi 4 75 equals 303 dollars total. This is approximately 3 percent of our total 10,000 dollar build budget. We achieve adequate navigation capability for cable-following missions at minimal cost. Adding the DVL upgrade increases navigation cost to 20,303 dollars - a 67-fold increase for sub-meter accuracy that's unnecessary for our operational concept.

    The key insight: navigation requirements depend entirely on mission type. For cable inspection where the cable is the reference frame, baseline IMU plus depth plus GPS is sufficient. For survey missions requiring absolute positioning, invest in DVL. The modular design allows either configuration.
  }
\end{frame}
...
\begin{itemize}
  ...
  \item Each integration amplifies sensor noise and bias:
        \begin{itemize}
          \item Random noise → angular error grows as $\sqrt{t}$
          \item Bias error → position drift grows linearly with $t$
        \end{itemize}
...
\note{
  ...
  IMU-based navigation underwater is an integration problem: we integrate angular rate for orientation and acceleration twice for position.
...
% \begin{frame}{Acoustic Communication Options}
%   \textbf{Underwater Communication Comparison:}

%   \begin{table}
%     \tiny
%     \begin{tabular}{llllll}
%       \toprule
%       \textbf{Model}           & \textbf{Frequency} & \textbf{Data Rate} & \textbf{Range} & \textbf{Power} & \textbf{Cost}  \\
%       \midrule
%       \textbf{EvoLogics 18/34} & 18-34 kHz          & 13.9 kbps          & 2,000m         & 20W TX         & \$12-15K       \\
%       EvoLogics 42/65          & 42-65 kHz          & 31.2 kbps          & 1,000m         & 20W TX         & \$12-15K       \\
%       LinkQuest UWM2000H       & 28-38 kHz          & 1.2 kbps           & 1,500m         & 8W TX          & \$5-8K         \\
%       Water Linked M64         & 31.25 kHz          & 64 bps             & 500m           & 2W TX          & \$2,300        \\
%       \midrule
%       \textbf{Baseline WiFi}   & 2.4/5 GHz          & 60 Mbps real       & 100m (surf)    & 0.5W           & \textbf{\$50}  \\
%       \bottomrule
%     \end{tabular}
%   \end{table}
...
%   \textbf{Acoustic Modem Use Cases:}
%   \begin{itemize}
%     \item \textbf{Real-time control:} Tethered ROV replacement (13.9 kbps adequate for telemetry)
%     \item \textbf{Multi-vehicle coordination:} AUV swarms requiring inter-vehicle comms
%     \item \textbf{Emergency abort:} Critical command transmission during mission
%   \end{itemize}

%   \vspace{0.5em}

%   \textbf{Our Mission Profile:} Autonomous pre-programmed waypoints + surface WiFi data dump
%   \begin{itemize}
%     \item \textcolor{red}{Acoustic modem NOT required for baseline operation}
%     \item Optional upgrade for enhanced safety (\$12K adds 10\% to total cost)
%   \end{itemize}

%   \note{
%     \textbf{Acoustic Communication Physics and Limitations:}

%     Radio frequency signals attenuate exponentially in seawater due to electrical conductivity. At 2.4 gigahertz WiFi frequency, penetration depth is under 1 meter. Optical communication requires line-of-sight and operates only in clear water under 100 meters range - useless in turbid coastal waters where subsea cables are located. Acoustic communication is the ONLY viable option for long-range underwater communication, propagating through sound waves at approximately 1,500 meters per second in seawater.

%     However, acoustic channels are fundamentally limited. The underwater acoustic channel exhibits severe multipath propagation (reflections off surface, seabed, and thermoclines), Doppler shift from vehicle motion and ocean currents, and high ambient noise from shipping, marine life, and wave action. These physical limitations constrain data rates to kilobits per second versus megabits per second for RF in air.

%     \textbf{EvoLogics S2C M 18/34 - Professional Standard:}

%     The EvoLogics 18/34 operates at 18 to 34 kilohertz frequency band providing 13.9 kilobits per second data rate. Why this frequency? Lower frequencies (below 10 kilohertz) propagate further but suffer more from ambient noise and require larger transducers. Higher frequencies (above 50 kilohertz) have higher bandwidth but shorter range due to absorption. The 18 to 34 kilohertz band balances range (2,000 meters) and data rate for AUV applications.

%     The modem provides 20 watts transmit power and 1.5 watts receive/idle power. It implements a TCP/IP stack over the acoustic link, enabling standard network protocols. Built-in USBL positioning option allows acoustic ranging for vehicle tracking. Cost is 12,000 to 15,000 dollars, weight approximately 2 kilograms.

%     \textbf{EvoLogics 42/65 - Higher Bandwidth, Shorter Range:}

%     The 42/65 model operates at 42 to 65 kilohertz providing 31.2 kilobits per second - 2.2 times faster than the 18/34. However, higher frequency reduces range to 1,000 meters due to increased absorption. Cost is similar. Use case: short-range, high-bandwidth applications like underwater docking or close-formation multi-vehicle operations.

%     \textbf{LinkQuest UWM2000H - Mid-Range Option:}

%     The LinkQuest modem operates at 28 to 38 kilohertz with 1.2 kilobits per second data rate (9,600 baud equivalent). Range is 1,500 meters. Power consumption is 8 watts transmit, 0.5 watts receive. Cost is 5,000 to 8,000 dollars - roughly half the EvoLogics price. The lower data rate is adequate for status telemetry and basic command/control. This is a good balance of performance and cost for applications requiring underwater communication but not high bandwidth.

%     \textbf{Water Linked M64 - Ultra-Robust Budget Option:}

%     The M64 provides only 64 bits per second - extremely low bandwidth. However, this ultra-low data rate achieves exceptional robustness in noisy environments. Range is 500 meters typical. Power is 2 watts transmit. Cost is 2,300 dollars. Use case: emergency abort commands or simple status pings where robustness matters more than bandwidth. 64 bps is adequate for "abort mission" (16 bytes per second) or GPS position report (8 to 10 bytes per second).

%     \textbf{WiFi Comparison - Why Surface Communication Wins:}

%     WiFi provides 60 megabits per second real throughput at the surface - 4,300 times faster than the EvoLogics 13.9 kilobits per second acoustic modem, at 240 times lower cost (50 dollars versus 12,000 dollars). The limitation is that WiFi requires surfacing, adding 4 minutes every 30 minutes. However, surfacing also enables GPS position reset, so the communication overhead aligns with navigation requirements.

%     \textbf{When is Acoustic Modem Justified:}

%     Three scenarios make acoustic modems worthwhile. First, real-time control: if replacing a tethered ROV with an untethered vehicle that still requires continuous operator control, the acoustic modem enables command/telemetry. 13.9 kilobits per second is adequate for joystick commands, vehicle status, and compressed sonar imagery. Second, multi-vehicle coordination: swarms of AUVs performing coordinated search or survey require inter-vehicle communication. Third, emergency abort: if the mission profile includes high-risk maneuvers (entering shipwrecks, navigating complex structures), the ability to send an abort command underwater provides safety margin.

%     \textbf{Our Mission Profile - Why Acoustic is NOT Required:}

%     Our vehicle follows pre-programmed GPS waypoints, operates autonomously underwater, surfaces every 30 minutes for WiFi data dump and GPS reset. No real-time control required. No multi-vehicle coordination. Emergency scenarios (thruster failure, leak detection) trigger autonomous abort surfacing using onboard logic, not external commands.

%     The acoustic modem represents 12,000 dollars cost - more than our entire baseline build cost excluding the Ping360 sonar. It adds 1.5 to 2 kilograms weight. For our autonomous cable-following mission, it provides NO operational benefit.

%     \textbf{Optional Upgrade Path:}

%     The modular design allows adding the EvoLogics modem later if operational experience shows the need. The Navigator flight controller provides UART serial ports for modem integration. However, for initial technical feasibility demonstration and cost-constrained deployments, the acoustic modem is optional rather than required. This is a key design decision that reduces build cost from 22,000 dollars to 10,000 dollars while maintaining adequate capability for the specified mission.
%   }
% \end{frame}
...
%   \note{
%     At the surface, WiFi provides 150 megabits per second bandwidth for downloading video and sensor data. The vehicle also acquires GPS fixes to reset navigation drift.
...
\begin{frame}{Thruster Selection - T200}
  ...
  \note{
    Four thruster options were evaluated. SeaBotix BTD150 has good efficiency but insufficient thrust: 28 newtons versus 50 newtons for T200. Maxon MT30 has 6000 meter depth rating and costs much more than T200 - unnecessary for 250 meter requirement. T500 provides 158 newtons thrust but draws over 1 kilowatt continuously and weighs 1.1 kilograms.

    T200 offers best thrust-to-cost ratio at 0.38 newtons per dollar. Four T200 thrusters provide 200 newtons total thrust for 162 newtons required - 23 percent safety margin. This margin accommodates deviation in drag coefficient calculations.
...
\begin{frame}{Battery Sizing}
  ...
  \note{
    ...
    Three Blue Robotics 18Ah batteries provide 799 watt-hours - 27 percent margin over requirement. This enables 2.5 hour missions at 314 watts average power. This configuration is proven in the existing BlueROV2 platform with integrated battery management system.
  }
\end{frame}
...
\begin{frame}[shrink]{Mass \& Cost Summary}
  ...
  \begin{table}
    ...
    \begin{tabular}{l r r l}
      ...
      \midrule
      \textbf{Total}        & \textbf{15.67 kg}  & \textbf{\$10640}   &                                    \\
      \bottomrule
    \end{tabular}
  \end{table}
  ...
  \begin{itemize}
    ...
    \item \textbf{Cost:} \$10640 base build, much cheaper
  ...
  \note{
    ...
    \textbf{(Cost Details)}
    The estimated build cost of 10640 dollars represents 20-25 percent of comparable commercial AUV systems (50-150K range). The Ping360 sonar is the single most expensive component at 2,750 dollars. Optional additions like the EvoLogics acoustic modem (12K) or Nortek DVL (20K) would increase total cost but are not required for basic cable inspection missions. The design prioritizes core components to minimize cost while maintaining technical performance.
  }
\end{frame}
...
\begin{frame}{T200 Thruster}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item 4× thrusters = 200N total
        \item Flooded brushless motor (pressure-tolerant)
        \item Thrust/cost: 0.38 N/\$
        \item 6-20× cheaper than alternatives
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{0.5em}
