...
\begin{frame}{Navigation System Component Options}
  ...
  \textbf{Baseline Cost: \$20,420} (Navigator + Bar30 + GPS + DVL + RPi4)

  \note{
    \textbf{Navigator Flight Controller - Component Justification:}

    The Navigator provides dual IMUs: ICM-42688-P with 2.8 millidegrees per second per root hertz gyro noise (industry-leading for MEMS), and ICM-20602 backup with 0.015 degrees per second per root hertz. Dual magnetometers (BMM150) provide heading reference with redundancy. The Navigator integrates directly with Raspberry Pi 4 via 40-pin GPIO header, eliminating external cabling and reducing failure points inside the pressure housing. At 220 dollars, it costs 27 percent less than Pixhawk 6C (300 dollars) while providing equivalent or better sensor specifications. The Navigator is the native platform for ArduSub autopilot with proven reliability in thousands of BlueROV2 deployments worldwide.

    \textbf{Bar30 Depth Sensor - Why Adequate for Our Mission:}

    The Bar30 uses the MS5837-30BA pressure sensor providing 2 millimeter depth resolution and 2 meter absolute accuracy. For cable inspection missions where we maintain 2 to 5 meter altitude above the seabed, 2 meter accuracy is entirely adequate - we're not doing bathymetric surveys requiring centimeter precision. The limitation is that the sensor gel can absorb water over multi-day deployments, causing drift. Our 2-hour missions with daily topside access allow drying the sensor, eliminating this concern. Industrial alternatives like Keller PA-7LD provide 0.3 meter accuracy and superior long-term stability but cost 2 to 6 times more (200 to 500 dollars). The Bar30's I2C interface and native ArduSub support make integration trivial. At 90 dollars, it's exceptional value for short-duration applications.
    ...
    Navigator 220 plus Bar30 90 plus GPS 35 plus Raspberry Pi 4 75 equals 420 dollars total. This is approximately 4.6 percent of our total 9,197 dollar build budget. We achieve adequate navigation capability for cable-following missions at minimal cost. Adding the DVL upgrade increases navigation cost to 20,420 dollars - a 49-fold increase for sub-meter accuracy that's unnecessary for our operational concept.
...
\begin{frame}{T200 Thruster}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item 4× thrusters = 206N total
        \item Flooded brushless motor (pressure-tolerant)
        \item Thrust/cost: 0.22 N/\$
        \item 3.5-10× cheaper than alternatives
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{0.5em}
