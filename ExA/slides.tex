...
\begin{frame}{ The Trade-off Triangle}
  \begin{center}
    \begin{tikzpicture}[scale=1.2]
      ...
      % Labels at corners
      \node[above] at (2,3.464) {\Large\textbf{Capabilities}};
      ...
      \node[right, text width=3cm, align=left] at (4.2,3) {
        Better Capabilities (higher speed, sensor equipment etc) – Often meaning an increase in both power draw (endurance) and weight (mass), but is also the most vital aspect of the design.
      };
      \node[left, text width=3cm, align=right] at (-0.2,3) {
        Longer Operating Time – Either reduce power draw (capabilities), or larger battery (mass)
      };
      \node[below, text width=4cm, align=center] at (2,-0.5) {
        $<$ 25 kg limits battery\\to ~4-5 kg max (endurance), also sacrifices equipment or payload (capabilities)
      };
    \end{tikzpicture}
  \end{center}
