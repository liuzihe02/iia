...
\begin{frame}{Underwater Communication Challenges}
  ...
  \note{
    Radio signals attenuate within meters underwater due to conductivity. Optical communication requires line-of-sight and operates under 100 meters in clear water, much shorter in turbid conditions. Acoustic is the only long-range option with ranges exceeding several kilometers, but has severe limitations: 1-10 kilobit per second data rates, high latency, and costs 12,000 dollars versus 310 dollars for WiFi plus Iridium.

    The design solution stores sensor data onboard during the mission. At the surface, WiFi provides high-bandwidth data transfer at 50 megabits per second. Iridium enables position reporting when beyond WiFi range. This avoids expensive acoustic hardware while maintaining operational capability.
  }
\end{frame}
...
\begin{frame}{Underwater Acoustic Link Budget}
  ...
  \note{
    Transmission loss has two components: spreading loss at 20 log R, and absorption loss alpha times R. At 500 meters, total loss is 55.5 decibels. Source level 180 dB plus array gain 10 dB gives received level 134.5 dB. With noise level 60 dB and required SNR 10 dB, link margin is 64.5 decibels - sufficient to handle multipath interference. However, we chose WiFi and Iridium instead due to cost - 12,000 dollars for acoustic versus 310 dollars.
  }
\end{frame}
