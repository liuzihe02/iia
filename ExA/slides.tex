...
\begin{frame}{Underwater Communication Challenges}
  ...
  \note{
    ...
    Acoustic communication is the only viable long-range option. Sound propagates well in water with ranges exceeding several kilometers. However, acoustic modems have severe limitations: data rates around 1 to 10 kilobits per second, latency of seconds due to slow sound speed (1500 meters per second), and multipath interference from reflections off the surface and seafloor. Cost is also prohibitive - acoustic modems cost 12,000 dollars versus 310 dollars for WiFi plus Iridium satellite.

    The design solution stores all sensor data onboard during the mission. High-resolution camera video and sonar scans are recorded to the Raspberry Pi storage. At the surface, WiFi provides high-bandwidth data transfer at 50 megabits per second. Iridium satellite enables position reporting and mission status updates when beyond WiFi range. This approach avoids expensive acoustic hardware while maintaining operational capability.
  }
\end{frame}
