...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[->, >=Stealth, shorten >=1pt, auto, node distance=3.2cm,
        ...
        Vehicle: \textcolor{green!60!black}{\textbf{GREEN}}\\[1pt]
        Pedestrian: \textcolor{red}{\textbf{RED}}\\[1pt]
        \texttt{output = 10001}\\[1pt]
        {\tiny (ignores \texttt{request})}
        };

    \end{tikzpicture}
    \caption{Improved TLC with lockout state \texttt{L}. Output bits as in Figure~\ref{fig:state-diagram}. After state \texttt{R}, the FSM holds vehicle GREEN for 10\,s in \texttt{L}, ignoring requests. \texttt{reset} returns to \texttt{G} from any state.}
