...
\begin{itemize}
    ...
    \item \textbf{Active-low inputs.} The push-buttons output 0 when pressed. The reset is handled via \texttt{negedge reset} in the sensitivity list for asynchronous behaviour, and \texttt{request} is checked against \texttt{1'b0}.
...
Both the basic and improved TLC designs were implemented on the Cyclone V FPGA, with all state transitions and timing verified on-board. The lockout state prevented continuous requests from blocking the vehicle lane, and fitted within the existing 2-bit encoding. Verilog behavioural entry made the improvement straightforward compared to a schematic redesign. Overall, the FPGA platform proved well-suited for rapid prototyping of digital control logic: design changes required only re-writing a few lines of Verilog and recompiling, with no hardware modifications. This flexibility, combined with the low resource usage (27 ALMs for the full controller), makes FPGAs a practical choice for deploying custom timing-critical controllers.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{images/compilation_report.png}
    \vspace{-0.5em}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{images/fpga_board.jpg}
    \vspace{-0.5em}
...
\vspace{-0.5em}
\lstinputlisting[style=verilog, caption={Testbench Verilog code for the basic TLC.}, label={lst:tb}]{tlc_tb.v}
